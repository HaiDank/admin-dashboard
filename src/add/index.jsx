...
const AddRecipe = () => {
  ...
  const starsElement = stars.map((star, i) =>
    <button key={i} onClick={() => { setRecipeData(prevData => { return { ...prevData, rating: (prevData.rating !== i + 1) ? i + 1 : 0 } }) }}>
  ...
  const ingredientsElement = recipeData.ingredients.map((ingredient, i) =>
    <div key={i} className='flex px-4 text-gray-600 cursor-pointer hover:line-through'
      onClick={() => {
